...
var parseTeamRankingPage = function (html) {
    ...
    return $('.ranked-team')
        ...
        var $el = $(el);
        // Позиция команды
        var positionText = $el.find('.position').text().trim();
        var place = parseInt(positionText.replace('#', '')) || 0;
        // Название команды
        var name = $el.find('.teamLine .name').text().trim();
        // Очки
        var pointsText = $el.find('.points').text().trim();
        var points = parseInt(pointsText.replace(/[^\d]/g, '')) || 0;
        // ID команды - из ссылки на профиль в блоке lineup-con
        var profileLink = $el.find('.lineup-con a.moreLink[href^="/team/"]').attr('href');
        var id = 0;
        if (profileLink) {
            var idMatch = profileLink.match(/\/team\/(\d+)/);
            if (idMatch && idMatch[1]) {
                id = parseInt(idMatch[1]);
            }
        }
        // Изменение позиции
        var changeText = $el.find('.change').text().trim();
        var change = 0;
        var isNew = false;
        if (changeText === 'New') {
            isNew = true;
