...
var parsePage = function (html) {
    ...
    $('.match-wrapper').each(function (i, el) {
        var $el = $(el);
        var id = Number($el.attr('data-match-id'));
        var live = $el.attr('live') === 'true';
        var stars = $el.find('.match-rating .fa-star:not(.faded)').length;
        var eventId = Number($el.attr('data-event-id')) || undefined;
        // Извлечение информации о командах
        var team1Id = Number($el.attr('team1'));
        var team2Id = Number($el.attr('team2'));
        ...
        var $matchInfo = $el.find('.match-info');
        var format;
        var date;
        $matchInfo.children().each(function (_, child) {
            var $child = $(child);
        ...
        var eventName;
        var eventElement = $el.find('.match-top .match-event');
